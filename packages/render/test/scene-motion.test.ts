...
import type { SceneMotion, SceneMotionDelta, SceneMotionFrame } from '../src/scene-motion.js';
 ...
 * it has to be all or nothing across all three. The test that matters is the
 * one where the node half would accept and the edge half would refuse.
 */
...
function delta(patch: Partial<SceneMotionDelta>): SceneMotionDelta {
  return patch;
}
...
describe('createSceneMotion', () => {
  ...
  it('is settled only when every half is', () => {
    const motion = seeded();
    motion.apply(delta({ bounds: box(-10, -110, 200, 10) }));
  ...
  it('leaves a half the delta does not name exactly as it was', () => {
    const motion = seeded();
    motion.apply(delta({ nodes: { added: [], removed: [], moved: [{ id: 'b', center: { x: 1, y: -100 } }] } }));
  ...
  it('shares one feel across the three halves', () => {
    ...
    quick.resync({ nodes: [{ id: 'a', center: { x: 0, y: 0 } }], edges: [], bounds: box(0, 0, 1, 1) });
    quick.apply(
      delta({
