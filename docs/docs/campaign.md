---
id: campaign
title: Campaign dataset
sidebar_position: 5
---

# Campaign dataset

`@dagr/campaign` is the data the demo draws: schema types for a tabletop
campaign, and a seeded generator that produces one. It is a private,
unpublished package with zero dependencies, `@dagr/graph` included, because it
...
campaign and not a random graph. The decision record is
[the campaign demo plan](https://github.com/prnt-design/dagr/blob/main/plans/2026-08-14-campaign-demo.md);
...
drawing, and it costs the pass real time on every relayout. So the
routed kinds are the ones meant for the layout pipeline, and the overlay kinds
are meant to draw as straight or lightly curved lines gated by zoom or hover,
which is what will keep the far view from being a wall of grey. Ranking is the
...
rather than trusted to the code that says it made them. The suite runs in two
registers on purpose: fixed counts the generator writes as literals (6 regions,
12 factions, 60 quests) are asserted exactly, and stochastic quantities are
asserted as bands wide enough to hold across seeds, so a claim like the loop
quota is a property of the generator rather than a memorized fact about one
campaign.
...
Today its one consumer is the demo page's text listing; the card tier the
overlay will draw takes the same rows, which is why the rows are the shape they
are.
