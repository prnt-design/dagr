...
- [x] **M4.6** (`@dagr/render`) Spring integrator: critically damped springs
  ...
  **THE LONG-FRAME OPINION `ribbon.ts` ASKED FOR IS THAT NO CLAMP IS NEEDED.**
  ...
  746 the decay underflows to zero in a double while `A + Bh` can be an
  ...
  **THE SUITE IS CHECKED AGAINST THE EQUATION AND NOT ONLY AGAINST ITSELF.** A
  ...
  asserts that the two agree at a small substep and that the agreement tightens
  by a hundredfold as the substep shrinks a hundredfold. The same reference then
  ...
  **IT LIVES IN `@dagr/render`, EXPORTED: the third option, taken for the second
  ...
  and the four shared checks in `validate.ts`, and nothing else. Neither is
